...
then
  ...
  echo
  echo
  echo -e $YELLOW"Now going into a shell in the Docker container - Zeppelin process is terminated!"$NOCOLOR
  echo
  echo
  /bin/bash
else
  ...
  echo
  echo
  echo -e $YELLOW"Now going into a shell in the Docker container - Zeppelin process is still running on "$BOLD"http://localhost:$ZEPPELIN_PORT"$NOBOLD$NOCOLOR
  echo
  jps
  echo
  /bin/bash
